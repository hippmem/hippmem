...
| 实体（Entity） | 查询实体 → 实体倒排表 | 按覆盖查询实体数分级：0.2 / 0.35 / 0.5（覆盖 1 / 2 / 3+ 个查询实体） |
| 主题（Topic） | 查询主题 → 主题倒排表 | 固定 0.15 |
| 时间（Temporal） | 当前时刻的时/日/周桶 → 时间倒排表 | 固定 0.3（"最近的事"） |
| 语义（dense） | 查询向量 vs 记忆向量（余弦相似度） | 1/(1+距离)；向量随写入持久化，重新打开引擎时本地重建 |
...
4. **实体覆盖修正**：多实体查询（查询含 ≥2 个实体，如"X 与 Y 什么关系"）中，覆盖 k 个查询实体的候选记忆乘以 `(1 + 0.2 × k/N)`——答案必然同时涉及双方实体，所以覆盖全部查询实体的记忆在同档内优先，只共享一个实体词的词面干扰项会被压下去。单实体查询不启用。
...
1. **分数不会无限上涨**：种子能量上限 1.0、传播能量限制 [0,1]、边强度上限 1.0、情境联结修正上限 ×1.5（联结强度封顶 1.0 × 系数 0.5）——检索分数的绝对上限约为 1.5。频繁使用让分数上升，但会收敛，不会"涨上天"。
2. **检索不会自我强化**：仅检索不产生任何正向信号（`signal="retrieve"` 不是正信号）——否则常被捞出的记忆会滚雪球。
3. **确认不会跨查询越权**：确认绑定查询的情境（实体/主题指纹），只在与该情境相交的查询里生效——无关查询借不到一分热度；多轮确认可累积，但始终只作用于情境内。
